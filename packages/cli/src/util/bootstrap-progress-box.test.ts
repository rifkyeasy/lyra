...
describe('mapBootstrapMarkerToStage', () => {
  test('maps every gateway bootstrap.ts marker', () => {
    ...
    expect(mapBootstrapMarkerToStage('installing lyra (0.24.7)')).toBe('lyra-install')
    expect(mapBootstrapMarkerToStage('installing lyra (git main)')).toBe('lyra-install')
...
describe('BootstrapProgressBox TTY mode', () => {
  ...
  test('markStage running auto-completes prior pending stages (handles skipped bun-install)', () => {
    ...
    box.markStage('lyra-install', 'running')
  ...
  test('honors custom label override', () => {
    ...
    const box = new BootstrapProgressBox({
      out,
      labels: { 'lyra-install': 'lyra 0.24.7 installed' },
    })
    box.start()
    expect(out.combined()).toContain('lyra 0.24.7 installed')
  })
