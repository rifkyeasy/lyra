...
import { BOOTSTRAP_STAGE_MARKERS } from 'lyra-gateway'
...
export interface BootstrapProgressBoxOpts {
  ...
  title?: string
  /** Per-stage label override. Useful for injecting the version into lyra-install. */
...
export function mapBootstrapMarkerToStage(marker: string): BootstrapStageId | null {
  ...
  if (m.startsWith(BOOTSTRAP_STAGE_MARKERS.lyraInstall)) return 'lyra-install'
  ...
  onStageEvent = (stage: BootstrapStageId, status: BootstrapStageStatus): void => {
    if (!this.box) {
      ...
      this.box = new BootstrapProgressBox({
        labels: { 'lyra-install': `lyra ${this.cliVersion} installed` },
      })
      this.box.start()
    }
