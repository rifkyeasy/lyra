 ...
 *   - `lyra gateway start` (interactive Touch ID flow → spawn detached)
 *   - `lyra` chat fallback when no sock is present (auto-spawn before
...
export interface SpawnGatewayDaemonOpts {
   ...
   * to `~/.lyra/agents/<id>/gateway.log` (truncated on each boot) so
...
export function resolveLocalBin(): string {
  ...
  return join(pkgRoot, 'bin', 'lyra-gateway-local')
}
...
export async function spawnGatewayDaemon(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    LYRA_AGENT_ID: opts.agentId,
    LYRA_CONFIG: opts.configPath,
  }
  const stdioMode = opts.stdio ?? 'log-file'

  // v0.21.12: when stdio is 'log-file' redirect daemon stdout+stderr to
  // ~/.lyra/agents/<id>/gateway.log (truncate-on-restart). Pre-fix this
  ...
  // reboot the daemon mid-session and `lyra gateway logs -f` only follows
