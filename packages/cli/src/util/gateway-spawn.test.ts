 ...
 * avoid spinning up the real `bun packages/gateway/bin/lyra-gateway-local`
 * because that pulls in the full Sui runtime + plugin stack.
 */
...
beforeEach(() => {
  workDir = mkdtempSync(join(tmpdir(), 'lyra-spawn-test-'))
})
...
writeFileSync(${JSON.stringify(envOut)}, JSON.stringify({
  agent: process.env.LYRA_AGENT_ID,
  config: process.env.LYRA_CONFIG,
}))
